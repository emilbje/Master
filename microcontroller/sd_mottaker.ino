...
// ===================== OpenLog (UART) =====================
static const int OPENLOG_RX_PIN = 8;   // ESP32 RX  <- OpenLog TXO (valgfritt)
static const int OPENLOG_TX_PIN = 7;   // ESP32 TX  -> OpenLog RXI (viktig)
static const uint32_t OPENLOG_BAUD = 115200;

HardwareSerial OpenLog(1);
static const bool DEBUG_TO_USB_SERIAL = true;
...
const unsigned long LOG_INTERVAL_MS = 10; // 100 Hz logging

// Madgwick tuning
const float MADGWICK_BETA = 0.08f;

// Accel “gate”: hvis |a| avviker mye fra 1g -> gyro-only den samplen
const float ACC_GATE_G = 0.45f;

// ---------- RASK GYRO-BIAS ----------
static const uint16_t GYRO_BIAS_SAMPLES = 250; // 200–400 typisk
...
unsigned long last_us=0, last_log_ms=0;
...
bool initOneIMU(uint8_t ch){
  ...
  ok &= wreg(REG_SMPLRT_DIV, 4);     // 1kHz/(1+4)=200Hz intern
  ok &= wreg(REG_CONFIG, 0x04);      // DLPF ~21Hz
...
// Leser én sample: accel(g) + gyro(dps). Inkl. din flip for i>=3.
bool readIMUSample(uint8_t i, float &ax, float &ay, float &az,
  ...
  // Din eksisterende “flip” for IMU >= 3: (180° rotasjon rundt z)
  // -> ax,ay,gx,gy inverteres; az og gz beholdes.
  if (i >= 3) {
    ax = -ax;  ay = -ay;
...
// Init quaternion fra accel (yaw=0)
...
void quatToRollPitchStable(uint8_t i, float &rollOut, float &pitchOut){
  ...
  // Alternative Euler branches for continuity
...
void stepIMU(uint8_t i, float dt){
  ...
  // Init quaternion fra accel når vi har en “OK” gravitasjonsmåling
  if (!q_init[i]){
  ...
  if (accelGood){
    madgwickUpdateIMU_core(i, gx, gy, gz, ax, ay, az, dt);
  } else {
    quatIntegrateGyroOnly(i, gx, gy, gz, dt);
  }
...
void logLine(const char* line){
  OpenLog.print(line);
  if (DEBUG_TO_USB_SERIAL) Serial.print(line);
}

// ---------- SETUP ----------
void setup(){
  if (DEBUG_TO_USB_SERIAL) {
    Serial.begin(115200);
    delay(150);
  }

  OpenLog.begin(OPENLOG_BAUD, SERIAL_8N1, OPENLOG_RX_PIN, OPENLOG_TX_PIN);
  delay(150);

  Wire.begin(SDA_PIN, SCL_PIN);
  Wire.setClock(400000);

  if (DEBUG_TO_USB_SERIAL) Serial.println("Boot: ABS ONLY (gyro bias fast + Madgwick 6DOF + stable Euler)");

  // Samme format som før:
  // p0..p7 = ROLL(ABS), r0..r7 = PITCH(ABS)
  OpenLog.println("time_ms\tp0\tp1\tp2\tp3\tp4\tp5\tp6\tp7\tr0\tr1\tr2\tr3\tr4\tr5\tr6\tr7");

  // Init IMUer
  for (uint8_t i=0; i<NIMU; i++){
    imu_ok[i] = initOneIMU(IMU_CH[i]);
    if (!imu_ok[i]){
      if (DEBUG_TO_USB_SERIAL) Serial.printf("IMU %u (ch %u) ikke funnet.\n", i, IMU_CH[i]);
      OpenLog.printf("IMU %u (ch %u) ikke funnet.\n", i, IMU_CH[i]);
    }
  }

  // RASK gyro bias (forutsetter skien ligger helt i ro)
  OpenLog.println("### Gyro bias fast (hold still) ###");
  if (DEBUG_TO_USB_SERIAL) Serial.println("### Gyro bias fast (hold still) ###");

  for (uint8_t i=0; i<NIMU; i++){
    if (!imu_ok[i]) continue;
    calibrateGyroFast(i, GYRO_BIAS_SAMPLES);
    OpenLog.printf("IMU%u bias dps: %.3f %.3f %.3f\n", i, gx_bias[i], gy_bias[i], gz_bias[i]);
    if (DEBUG_TO_USB_SERIAL) Serial.printf("IMU%u bias dps: %.3f %.3f %.3f\n", i, gx_bias[i], gy_bias[i], gz_bias[i]);
  }

  // Init quaternion fra accel (kort vindu, ikke stå fast)
  uint32_t t0 = millis();
  while (millis() - t0 < 1200){
    bool allInit = true;
    for (uint8_t i=0;i<NIMU;i++){
      if (!imu_ok[i]) continue;
      if (q_init[i]) continue;
  ...
  OpenLog.println("### Start: ABS angles (no zero) ###");
  if (DEBUG_TO_USB_SERIAL) Serial.println("### Start: ABS angles (no zero) ###");
...
void loop(){
  ...
  if (millis() - last_log_ms >= LOG_INTERVAL_MS){
    ...
    idx += snprintf(line+idx, sizeof(line)-idx, "%lu\t", (unsigned long)millis());

    // p0..p7 = ROLL ABS
    for (uint8_t i=0;i<NIMU;i++){
      float v = (!imu_ok[i] || !q_init[i]) ? NAN : roll_deg[i];
      idx += snprintf(line+idx, sizeof(line)-idx, "%.1f\t", isnan(v) ? -999.9f : v);
      if (idx >= sizeof(line)) break;
    }

    // r0..r7 = PITCH ABS
    for (uint8_t i=0;i<NIMU;i++){
      float v = (!imu_ok[i] || !q_init[i]) ? NAN : pitch_deg[i];
